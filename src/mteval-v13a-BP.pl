...
sub score_system
{
	...
	if ($method eq "BLEU")
	{
        ...
        print "Hypothesis length: $cum_tst_cnt[1]\n";
        print "Reference length: $cum_ref_length\n";
        my ($ratio, $bp);
        $ratio = $cum_tst_cnt[1] / $cum_ref_length;
        $bp = exp( min (0, 1 - $cum_ref_length / $cum_tst_cnt[1] ) ) if ( $cum_tst_cnt[1] > 0 );
        print "Ratio: $ratio\n";
